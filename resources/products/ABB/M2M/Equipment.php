...
class Equipment extends EquipmentModel implements EquipmentInterface
{
    ...
    public function refresh() {
        ...
        $response = $client -> readHoldingRegisters($this->slave, 4097, 54);
        $endianness = false;
        if ($response->success()) {
            ...
            $this->updateVariable('voltage12', $response->getData()->withEndianness($endianness)->readUint32(1));
            $this->updateVariable('voltage23', $response->getData()->withEndianness($endianness)->readUint32(3));
            $this->updateVariable('voltage31', $response->getData()->withEndianness($endianness)->readUint32(5));
            $this->updateVariable('voltage1N', $response->getData()->withEndianness($endianness)->readUint32(7));
            $this->updateVariable('voltage2N', $response->getData()->withEndianness($endianness)->readUint32(9));
            $this->updateVariable('voltage3N', $response->getData()->withEndianness($endianness)->readUint32(11));
    ...
    public function test()
    {
        $client = new ModbusClient();
        $client->connect($this->address_ip, $this->port);
        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 4097, 74);
        $endianness = false;
