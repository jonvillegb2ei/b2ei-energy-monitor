...
namespace Product\Legrand\ContoD4;
...
class Equipment extends EquipmentModel implements EquipmentInterface
{
    ...
    public function refresh() {
        ...
        $response = $client -> readHoldingRegisters($this->slave, 4096, 62);
        $endianness = false;
        if ($response->success()) {
            ...
            $this->updateVariable('active_power', $response->getData()->withEndianness($endianness)->readInt32(21)/100);
            $this->updateVariable('active_power1', $response->getData()->withEndianness($endianness)->readInt32(45)/100);
            $this->updateVariable('active_power2', $response->getData()->withEndianness($endianness)->readInt32(47)/100);
            $this->updateVariable('active_power3', $response->getData()->withEndianness($endianness)->readInt32(49)/100);
            $this->updateVariable('reactive_power', $response->getData()->withEndianness($endianness)->readInt32(23)/100);
            $this->updateVariable('reactive_power1', $response->getData()->withEndianness($endianness)->readInt32(54)/100);
            $this->updateVariable('reactive_power2', $response->getData()->withEndianness($endianness)->readInt32(56)/100);
            $this->updateVariable('reactive_power3', $response->getData()->withEndianness($endianness)->readInt32(58)/100);
            $this->updateVariable('apparent_power', $response->getData()->withEndianness($endianness)->readUint32(25)/100);
            $this->updateVariable('active_energy', $response->getData()->withEndianness($endianness)->readUint32(29)/100);
            $this->updateVariable('reactive_energy', $response->getData()->withEndianness($endianness)->readUint32(31)/100);
    ...
    public function getWidgetVariablesAttribute()
    {
        return $this->variables()
            ->whereIn('name',['active_power','reactive_power','active_energy'])
            ->orderBy('id','ASC')
            ->get();
    }
    ...
    public function test()
    {
        $client = new ModbusClient();
        $client->connect($this->address_ip, $this->port);
        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 4096, 62);
        $endianness = false;
