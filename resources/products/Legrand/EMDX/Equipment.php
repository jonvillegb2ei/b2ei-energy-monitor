...
namespace Product\Legrand\EMDX;
...
class Equipment extends EquipmentModel implements EquipmentInterface
{
    ...
    public function refresh() {
        ...
        $response = $client -> readHoldingRegisters($this->slave, 4096, 74);
        $endianness = false;
        if ($response->success()) {

            $this->updateVariable('current1', $response->getData()->withEndianness($endianness)->readUint32(7)/1000);
            $this->updateVariable('current2', $response->getData()->withEndianness($endianness)->readUint32(9)/1000);
            $this->updateVariable('current3', $response->getData()->withEndianness($endianness)->readUint32(11)/1000);
            $this->updateVariable('currentN', $response->getData()->withEndianness($endianness)->readUint32(13)/1000);
            $this->updateVariable('voltage12', $response->getData()->withEndianness($endianness)->readUint32(15)/1000);
            $this->updateVariable('voltage23', $response->getData()->withEndianness($endianness)->readUint32(17)/1000);
            $this->updateVariable('voltage31', $response->getData()->withEndianness($endianness)->readUint32(19)/1000);
            $this->updateVariable('voltage1N', $response->getData()->withEndianness($endianness)->readUint32(1)/1000);
            $this->updateVariable('voltage2N', $response->getData()->withEndianness($endianness)->readUint32(3)/1000);
            $this->updateVariable('voltage3N', $response->getData()->withEndianness($endianness)->readUint32(5)/1000);
            $this->updateVariable('power_factor', $response->getData()->withEndianness($endianness)->readInt16(37)/100);
            $this->updateVariable('active_power', $response->getData()->withEndianness($endianness)->readInt32(21));
            $this->updateVariable('active_power1', $response->getData()->withEndianness($endianness)->readInt32(45));
            $this->updateVariable('active_power2', $response->getData()->withEndianness($endianness)->readInt32(47));
            $this->updateVariable('active_power3', $response->getData()->withEndianness($endianness)->readInt32(49));
            $this->updateVariable('reactive_power', $response->getData()->withEndianness($endianness)->readInt32(23));
            $this->updateVariable('reactive_power1', $response->getData()->withEndianness($endianness)->readInt32(54));
            $this->updateVariable('reactive_power2', $response->getData()->withEndianness($endianness)->readInt32(56));
            $this->updateVariable('reactive_power3', $response->getData()->withEndianness($endianness)->readInt32(58));
            $this->updateVariable('apparent_power', $response->getData()->withEndianness($endianness)->readUint32(25));
            $this->updateVariable('apparent_power1', $response->getData()->withEndianness($endianness)->readUint32(63));
            $this->updateVariable('apparent_power2', $response->getData()->withEndianness($endianness)->readUint32(65));
            $this->updateVariable('apparent_power3', $response->getData()->withEndianness($endianness)->readUint32(67));
            $this->updateVariable('active_energy', $response->getData()->withEndianness($endianness)->readUint32(29)/1000);
            $this->updateVariable('reactive_energy', $response->getData()->withEndianness($endianness)->readUint32(31)/1000);
            $this->updateVariable('frequency', $response->getData()->withEndianness($endianness)->readUint16(39)/10);
        }
    }
    ...
    public function test()
    {
        $client = new ModbusClient();
        $client->connect($this->address_ip, $this->port);
        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 4096, 74);
        $endianness = false;
        ...
        $current = $response->getData()->withEndianness($endianness)->readUint32(7);
        $output = sprintf("\ncurrent 1: %.2f A", $current/1000);
        $current = $response->getData()->withEndianness($endianness)->readUint32(9);
        $output .= sprintf("\ncurrent 2: %.2f A", $current/1000);
        $current = $response->getData()->withEndianness($endianness)->readUint32(11);
        $output .= sprintf("\ncurrent 3: %.2f A", $current/1000);
        $current = $response->getData()->withEndianness($endianness)->readUint32(13);
        $output .= sprintf("\ncurrent N: %.2f A", $current/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(15);
        $output .= sprintf("\nvoltage 12: %.2f VAC", $voltage/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(17);
        $output .= sprintf("\nvoltage 23: %.2f VAC", $voltage/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(19);
        $output .= sprintf("\nvoltage 31: %.2f VAC", $voltage/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(1);
        $output .= sprintf("\nvoltage 1N: %.2f VAC", $voltage/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(3);
        $output .= sprintf("\nvoltage 2N: %.2f VAC", $voltage/1000);
        $voltage = $response->getData()->withEndianness($endianness)->readUint32(5);
        $output .= sprintf("\nvoltage 3N: %.2f VAC", $voltage/1000);
        $power = $response->getData()->withEndianness($endianness)->readInt16(37);
        $output .= sprintf("\npower factor: %.2f Pi", $power/100);
        $power = $response->getData()->withEndianness($endianness)->readInt32(21);
        $output .= sprintf("\nactive power total: %.2f kW", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(45);
        $output .= sprintf("\nactive power L1: %.2f kW", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(47);
        $output .= sprintf("\nactive power L2: %.2f kW", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(49);
        $output .= sprintf("\nactive power L3: %.2f kW", $power);
     	$power = $response->getData()->withEndianness($endianness)->readInt32(23);
        $output .= sprintf("\nreactive power total: %.2f kVAR", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(54);
        $output .= sprintf("\nreactive power L1: %.2f kVAR", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(56);
        $output .= sprintf("\nreactive power L2: %.2f kVAR", $power);
        $power = $response->getData()->withEndianness($endianness)->readInt32(58);
        $output .= sprintf("\nreactive power L3: %.2f kVAR", $power);
        $power = $response->getData()->withEndianness($endianness)->readUint32(25);
        ...
        $power = $response->getData()->withEndianness($endianness)->readUint32(65);
        $output .= sprintf("\napparent power L2: %.2f kVA", $power);
        $power = $response->getData()->withEndianness($endianness)->readUint32(67);
        $output .= sprintf("\napparent power L3: %.2f kVA", $power);
        $power = $response->getData()->withEndianness($endianness)->readUint32(29);
        $output .= sprintf("\nactive energy: %.2f kWh", $power/1000);
        $power = $response->getData()->withEndianness($endianness)->readUint32(31);
        $output .= sprintf("\nreactive energy: %.2f kWh", $power/1000);
        $frequency = $response->getData()->withEndianness($endianness)->readUint16(39);
