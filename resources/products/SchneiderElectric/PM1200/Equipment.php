...
namespace Product\SchneiderElectric\PM1200;
...
class Equipment extends EquipmentModel implements EquipmentInterface
{
    ...
    public function refresh() {
        ...
        $response = $client -> readHoldingRegisters($this->slave, 3912, 120);
        $endianness = false;
        if ($response->success()) {
            ...
            $this->updateVariable('currentAvg', $response->getData()->withEndianness($endianness)->readFloat32(7));
    ...
    public function test()
    {
        $client = new ModbusClient();
        $client->connect($this->address_ip, $this->port);
        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 3902, 60);
        $endianness = false;
...
//        $output = sprintf("states: %s %s", $states[0] ? 'ON' : 'OFF', ($states[1] or $states[2]) ? 'FAULT' : 'OK');
        ...
        $current = $response->getData()->withEndianness($endianness)->readFloat32(26);
        $output .= sprintf("\ncurrent 1: %.2f A", $current);

        $current = $response->getData()->withEndianness($endianness)->readFloat32(40);
        $output .= sprintf("\ncurrent 2: %.2f A", $current);

        $current = $response->getData()->withEndianness($endianness)->readFloat32(54);
        $output .= sprintf("\ncurrent 3: %.2f A", $current);

        $voltage = $response->getData()->withEndianness($endianness)->readFloat32(22);
        $output .= sprintf("\nvoltage 12: %.2f VAC", $voltage);

        $voltage = $response->getData()->withEndianness($endianness)->readFloat32(36);
        $output .= sprintf("\nvoltage 23: %.2f VAC", $voltage);

        $voltage = $response->getData()->withEndianness($endianness)->readFloat32(50);
        ...
        $voltage = $response->getData()->withEndianness($endianness)->readFloat32(38);
        $output .= sprintf("\nvoltage 2N: %.2f VAC", $voltage);

        $voltage = $response->getData()->withEndianness($endianness)->readFloat32(52);
        $output .= sprintf("\nvoltage 3N: %.2f VAC", $voltage);

        $frequency = $response->getData()->withEndianness($endianness)->readFloat32(12);
        $output .= sprintf("\nfrequency: %.2f Hz", $frequency);

        $power_factor = $response->getData()->withEndianness($endianness)->readFloat32(4);
        $output .= sprintf("\npower factor: %.2f PI", $power_factor);


        $responseAvg = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 3000, 20);
        $endianness = false;
        if ($response->hasException())
            throw $response->getException();
        if (!$response->success())
...
//
//        $power = $response->getData()->withEndianness($endianness)->readFloat32(0);
//        $output .= sprintf("\nreactive power total: %.2f kVAR", $power);

        $frequency = $responseAvg->getData()->withEndianness($endianness)->readFloat32(14);
        $output .= sprintf("\nfrequency2: %.2f Hz", $frequency);
