...
namespace Product\Socomec\CountisE23;
...
class Equipment extends EquipmentModel implements EquipmentInterface
{
    ...
    public function refresh() {
        ...
        $response = $client -> readHoldingRegisters($this->slave, 50514, 50);
        $endianness = false;
        if ($response->success()) {
            $this->updateVariable('current1', $response->getData()->withEndianness($endianness)->readUint32(15)/1000);
            $this->updateVariable('current2', $response->getData()->withEndianness($endianness)->readUint32(17)/1000);
            $this->updateVariable('current3', $response->getData()->withEndianness($endianness)->readUint32(19)/1000);
            $this->updateVariable('currentN', $response->getData()->withEndianness($endianness)->readUint32(21)/1000);
            $this->updateVariable('voltage12', $response->getData()->withEndianness($endianness)->readUint32(1)/100);
            $this->updateVariable('voltage23', $response->getData()->withEndianness($endianness)->readUint32(3)/100);
            $this->updateVariable('voltage31', $response->getData()->withEndianness($endianness)->readUint32(5)/100);
            $this->updateVariable('voltage1N', $response->getData()->withEndianness($endianness)->readUint32(7)/100);
            $this->updateVariable('voltage2N', $response->getData()->withEndianness($endianness)->readUint32(9)/100);
            $this->updateVariable('voltage3N', $response->getData()->withEndianness($endianness)->readUint32(11)/100);
            $this->updateVariable('frequency', $response->getData()->withEndianness($endianness)->readUint32(13)/1000);
            $this->updateVariable('active_power1', $response->getData()->withEndianness($endianness)->readInt32(31)/10);
            $this->updateVariable('active_power2', $response->getData()->withEndianness($endianness)->readInt32(33)/10);
            $this->updateVariable('active_power3', $response->getData()->withEndianness($endianness)->readInt32(35)/10);
            $this->updateVariable('active_power', $response->getData()->withEndianness($endianness)->readInt32(23)/10);
            $this->updateVariable('reactive_power1', $response->getData()->withEndianness($endianness)->readInt32(37)/10);
            $this->updateVariable('reactive_power2', $response->getData()->withEndianness($endianness)->readInt32(39)/10);
            $this->updateVariable('reactive_power3', $response->getData()->withEndianness($endianness)->readInt32(41)/10);
            $this->updateVariable('reactive_power', $response->getData()->withEndianness($endianness)->readInt32(25)/10);
            $this->updateVariable('apparent_power1', $response->getData()->withEndianness($endianness)->readUint32(43)/10);
            $this->updateVariable('apparent_power2', $response->getData()->withEndianness($endianness)->readUint32(45)/10);
            $this->updateVariable('apparent_power3', $response->getData()->withEndianness($endianness)->readUint32(47)/10);
            $this->updateVariable('apparent_power', $response->getData()->withEndianness($endianness)->readUint32(27)/10);
            $this->updateVariable('power_factor', $response->getData()->withEndianness($endianness)->readInt32(29)/1000);

            $response = $client -> readHoldingRegisters($this->slave, 50770, 8);
            $endianness = false;
            if ($response->success()) {
                $this->updateVariable('active_energy', $response->getData()->withEndianness($endianness)->readUint32(1)/1000);
                $this->updateVariable('reactive_energy', $response->getData()->withEndianness($endianness)->readUint32(3)/1000);
                $this->updateVariable('apparent_energy', $response->getData()->withEndianness($endianness)->readUint32(5)/1000);
            }
        }
    }
    ...
    public function test()
    {
        $client = new ModbusClient();
        $client->connect($this->address_ip, $this->port);
        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 50514, 50);
        $endianness = false;
        ...
        $current = $response->getData()->withEndianness($endianness)->readUint32(15);
        ...
        $frequency = $response->getData()->withEndianness($endianness)->readUint32(13);
        $output .= sprintf("\nfrequency: %.2f Hz", $frequency/1000);
        $power = $response->getData()->withEndianness($endianness)->readInt32(31);
        $output .= sprintf("\nactive power L1: %.2f kW", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readInt32(33);
        ...
        $power = $response->getData()->withEndianness($endianness)->readInt32(39);
        $output .= sprintf("\nreactive power L2: %.2f kVAR", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readInt32(41);
        $output .= sprintf("\nreactive power L3: %.2f kVAR", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readInt32(25);
        ...
        $power = $response->getData()->withEndianness($endianness)->readUint32(45);
        $output .= sprintf("\napparent power L2: %.2f kVA", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readUint32(47);
        $output .= sprintf("\napparent power L3: %.2f kVA", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readUint32(27);
        $output .= sprintf("\napparent power total: %.2f kVA", $power/10);
        $power = $response->getData()->withEndianness($endianness)->readInt32(29);
        $output .= sprintf("\npower factor: %.2f Pi", $power/1000);



        $response = $client -> readHoldingRegisters($this->slave == 0 ? 255 : $this->slave, 50770, 8);
        $endianness = false;
        if ($response->hasException())
            throw $response->getException();
        if (!$response->success())
        ...
        $power = $response->getData()->withEndianness($endianness)->readUint32(3);
        $output .= sprintf("\nreactive energy: %.2f kWh", $power/1000);
        $power = $response->getData()->withEndianness($endianness)->readUint32(5);
